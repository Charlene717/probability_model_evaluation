...
for (i in 1:20) {
  ...
  n_sampling = i*20 # n_sampling = 100
  t_sampling = 100
  Mean1 = 0
  Mean2 = 0.3
  ...
  if(i==1){
    ...
    ACC_Sum_N.df <- data.frame(n_sampling = n_sampling,
                             t_sampling = t_sampling,
                             Mean = Mean2,
                             C1_Ori=Case1_OC.lt$ACC,
                             C1_Bon=Case1_OC_Bon.lt$ACC,
    ...
    ACC_Sum.df <- rbind(ACC_Sum.df,ACC_Sum_N.df)
  }
  ...
  ACC_Sum_T.df <- select(ACC_Sum.df,c(1,4:9)) %>% gather(Type,ACC,-n_sampling)
  ggplot(ACC_Sum_T.df, aes(x = n_sampling, y = ACC, colour = Type)) + 
    geom_line(size=3) +  #linetype = "dashed",
